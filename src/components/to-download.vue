    ...
    >
      <el-tab-pane label=" 概况">
        ...
        <p-updown-chart
          v-if="this.$props.todo.status==='active'"
          ref="chart"
          style="height: 150px;width:calc(100% - 40px);position: absolute"
          :ups="this.ups"
          :downs="this.downs"
        ></p-updown-chart>
        ...
        <el-table
          ...
          :style="{'margin-top':this.$props.todo.status!=='active'?'0':'150px'}"
          stripe
        >
          <el-table-column
            prop="name"
    ...
    methods: {
      ...
      getInfoTable() {
        let re = [
          ...
          {
            name: "任务状态",
            value: "<span title='" + this.$props.todo.errorMessage + "'>" + this.parseState(this.$props.todo.status, this.$props.todo) + "</span>"
          },
        ...
        if (this.$props.todo.bittorrent) {
          ...
          re.push({
            name: "磁力链接",
            value: "<a href='" + magnet + "' target='_blank' style='text-decoration: none;'>" + magnet + "</a>"
          });
          re.push({
            name: "下载宣言",
            value: "<textarea style='width:100%;resize:none;' rows=5>" + this.$props.todo.bittorrent.announceList.map(it => it[0]).join("\n") + "</textarea>"
          });
        } else {
          re.push({
            name: "链接地址",
            value: "<a href='" + this.$props.todo.files[0].uris[0].uri + "' target='_blank' style='text-decoration: none;'>" + this.$props.todo.files[0].uris[0].uri + "</a>"
          });
        }
        return re;
      },
